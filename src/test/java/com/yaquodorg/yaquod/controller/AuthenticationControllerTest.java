...
class AuthenticationControllerTest {
  ...
  void shouldRefreshTokenSuccessfully() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/auth/token-refresh").header("Authorization", "Bearer valid-refresh-token"))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.data.accessToken").value("access-token"));
  ...
  void shouldReturn401WithExpiredToken() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/auth/token-refresh").header("Authorization", "Bearer expired-token"))
        .andDo(print())
  ...
  void shouldReturn500OnUnexpectedErrorDuringRefresh() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/auth/token-refresh").header("Authorization", "Bearer token"))
        .andDo(print())
        .andExpect(status().isInternalServerError())
        .andExpect(jsonPath("$.success").value(false))
