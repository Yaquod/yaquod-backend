 ...
 *
 * <p>
 * Integration tests for VehicleController Tests full Spring context including
 * security Uses real database (H2 or Testcontainers)
 */
@SpringBootTest
...
class VehicleControllerSecurityIntegrationTest {
    ...
    void setUp() {
        ...
        createVehicleDto = CreateVehicleDto.builder().vinNumber(VinNumber1).plateNo("ABC-123").color("RED")
                .carCompany("Toyota").model("Camry").seats(4).build();
    ...
    void shouldReturn401WhenNotAuthenticated() throws Exception {
        mockMvc.perform(post("/api/vehicles").contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(createVehicleDto))).andDo(print())
    ...
    void shouldGetVehiclesWithAdminRole() throws Exception {
        // Act & Assert
        mockMvc.perform(get("/api/vehicles")).andDo(print()).andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data", hasSize(0)));
    }
    ...
    void shouldRequireAdminRoleForGetByVIN() throws Exception {
        mockMvc.perform(get("/api/vehicles/vin/test-vin")).andDo(print()).andExpect(status().isForbidden());
    }
    ...
    void shouldRequireAdminRoleForUpdate() throws Exception {
        mockMvc.perform(patch("/api/vehicles").contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(createVehicleDto))).andDo(print())
                .andExpect(status().isForbidden());
    }
    ...
    void shouldRequireAdminRoleForDelete() throws Exception {
        mockMvc.perform(delete("/api/vehicles/id/1")).andDo(print()).andExpect(status().isForbidden());
    }
    ...
    void shouldRequireAdminRoleForLocationUpdate() throws Exception {
        mockMvc.perform(patch("/api/vehicles/vin/test-vin/location-update")).andDo(print())
                .andExpect(status().isForbidden());
    }
    ...
    void shouldRequireAdminRoleForStatusUpdate() throws Exception {
        mockMvc.perform(patch("/api/vehicles/vin/test-vin/status-update")).andDo(print())
                .andExpect(status().isForbidden());
    }
    ...
    void shouldPerformFullCrudFlow() throws Exception {
        ...
        String createResponse = mockMvc
                .perform(post("/api/vehicles").contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(createVehicleDto)))
                .andDo(print()).andExpect(status().isCreated()).andExpect(jsonPath("$.success").value(true)).andReturn()
        ...
        // 2. Get vehicle by ID
        mockMvc.perform(get("/api/vehicles/id/{vehicleId}", vehicleId)).andDo(print()).andExpect(status().isOk())
        ...
        // 4. Update vehicle
        CreateVehicleDto updateDto = CreateVehicleDto.builder().vinNumber(VinNumber1).plateNo("XYZ-999")
                .color("Updated Color").carCompany("Updated Company").model("Updated Model").seats(6).build();

        mockMvc.perform(patch("/api/vehicles").contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(updateDto))).andDo(print()).andExpect(status().isOk());

        // 5. Delete vehicle
        mockMvc.perform(delete("/api/vehicles/id/{vehicleId}", vehicleId)).andDo(print()).andExpect(status().isOk())
    ...
    void shouldHandleConcurrentVehicleCreation() throws Exception {
        ...
        mockMvc.perform(post("/api/vehicles").contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(createVehicleDto))).andExpect(status().isCreated());

        // Create second vehicle with different plate
        CreateVehicleDto dto2 = CreateVehicleDto.builder().vinNumber(VinNumber2).plateNo("XYZ-789").color("WHITE")
                .carCompany("Honda").model("Accord").seats(5).build();

        mockMvc.perform(post("/api/vehicles").contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(dto2))).andExpect(status().isCreated());

        // Verify both exist
        mockMvc.perform(get("/api/vehicles")).andExpect(status().isOk()).andExpect(jsonPath("$.data", hasSize(2)));
    }
    ...
    void shouldGetVehicleByVINAfterCreation() throws Exception {
        ...
        // Get by VIN
        mockMvc.perform(get("/api/vehicles/vin/{vinNumber}", vin)).andDo(print()).andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.vinNumber").value(vin))
                .andExpect(jsonPath("$.data.plateNo").value("ABC-123"));
    }
    ...
    void shouldReturn400ForNonExistentVehicleById() throws Exception {
        mockMvc.perform(get("/api/vehicles/id/999")).andDo(print()).andExpect(status().isBadRequest())
    ...
    void shouldReturn400ForNonExistentVehicleByVIN() throws Exception {
        mockMvc.perform(get("/api/vehicles/vin/non-existent-vin")).andDo(print()).andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success").value(false));
    }
    ...
    void shouldSendLocationUpdateViaMQTT() throws Exception {
        ...
        mockMvc.perform(patch("/api/vehicles/vin/{vinNumber}/location-update", vin)).andDo(print())
    ...
    void shouldSendStatusUpdateViaMQTT() throws Exception {
        ...
        mockMvc.perform(patch("/api/vehicles/vin/{vinNumber}/status-update", vin)).andDo(print())
                .andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.message").value("Order signal sent!"));
    }
    ...
    void mqttUpdatesShouldRequireAdminRole() throws Exception {
        // Location update
        mockMvc.perform(patch("/api/vehicles/vin/test-vin/location-update")).andExpect(status().isForbidden());

        // Status update
        mockMvc.perform(patch("/api/vehicles/vin/test-vin/status-update")).andExpect(status().isForbidden());
    }
    ...
    void shouldValidateDtoFieldsOnCreate() throws Exception {
        ...
        CreateVehicleDto invalidDto = CreateVehicleDto.builder().plateNo("") // Invalid
                .model(null) // Invalid
                .seats(-1) // Invalid
                .build();

        mockMvc.perform(post("/api/vehicles").contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(invalidDto))).andDo(print())
    ...
    void shouldHandleMalformedJson() throws Exception {
        mockMvc.perform(post("/api/vehicles").contentType(MediaType.APPLICATION_JSON).content("{invalid json"))
    ...
    void shouldHandleMissingContentType() throws Exception {
        mockMvc.perform(post("/api/vehicles").content(objectMapper.writeValueAsString(createVehicleDto))).andDo(print())
    ...
    void shouldHandleMultipleRapidRequests() throws Exception {
        ...
        for (int i = 0; i < 2; i++) {
            CreateVehicleDto dto = CreateVehicleDto.builder().vinNumber(validVins[i]).plateNo("PLATE-" + i)
                    .carCompany("Company" + i).color("Color" + i).model("Model " + i).seats(4).build();
    ...
    void shouldHandleSpecialCharactersInPlateNumbers() throws Exception {
        CreateVehicleDto dto = CreateVehicleDto.builder().vinNumber(VinNumber2).plateNo("ABC-123!@#").color("RED")
                .carCompany("Test").model("Model").seats(4).build();
    ...
    void shouldPersistDataAcrossRequests() throws Exception {
        ...
        mockMvc.perform(get("/api/vehicles")).andExpect(status().isOk()).andExpect(jsonPath("$.data", hasSize(1)));
