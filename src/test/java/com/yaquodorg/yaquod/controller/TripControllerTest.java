...
class TripControllerTest {
  ...
  void shouldGetRequestStatus() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/trips/request/status/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.data.id").value(1))
        .andExpect(jsonPath("$.data.status").value("PENDING"));
  ...
  void shouldReturnBadRequestWhenRequestNotFound() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/trips/request/status/999"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(
            jsonPath("$.message").value("Failed to check Request status: Request not found!"));
  }
  ...
  void shouldGetTripByRequestId() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/trips/by-request/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.data.id").value(1))
  ...
  void shouldReturnBadRequestWhenTripNotFoundByRequestId() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/trips/by-request/999"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(
            jsonPath("$.message")
  ...
  void shouldReturnBadRequestWhenTripNotFoundById() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(
            jsonPath("$.message").value("Failed to get Trip by id: Trip not found for id: 999"));
  }
  ...
  void shouldGetTripsByVinNumber() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/trips/vehicle/VIN123456"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.data").isArray())
        .andExpect(jsonPath("$.data.length()").value(1))
  ...
  void shouldReturnBadRequestWhenGetTripsByVinNumberFails() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(
            jsonPath("$.message").value("Failed to get Trips by VIN number: Vehicle not found"));
  }
}
