...
public class RequestServiceImpl implements RequestService {
  ...
  public void updateRequest(
      Long requestId, RequestStatus requestStatus, double estimatedTime, double estimatedFare) {
    log.info(
  ...
  public void declineRequestById(Long id, Long userId) {
    ...
    if (request.getStatus() != RequestStatus.COMPLETED) {
      log.warn(
          "Request id: {} is not in COMPLETED state, current status: {}", id, request.getStatus());
      throw new IllegalStateException("Request is not in COMPLETED state");
    }
    if (trip.getStatus() != TripStatus.INITIATED) {
      log.warn(
          "Trip id: {} is not in INITIATED state, current status: {}", tripId, trip.getStatus());
