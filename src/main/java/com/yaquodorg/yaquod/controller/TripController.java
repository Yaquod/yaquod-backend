...
public class TripController {
  ...
  @Operation(
      ...
      description =
          "Creates a new trip request with start and end coordinates for the authenticated user")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<ApiResponse<Trip>> getTripById(
      @Parameter(description = "The unique database ID of the trip", required = true) @PathVariable
          Long tripId) {
    try {
  ...
  public ResponseEntity<ApiResponse<MessageResponse>> deleteTripById(
    ...
    try {
      ...
      return ResponseEntity.badRequest()
          .body(
              ApiResponse.createFailureResponse("Failed to delete Trip by id: " + e.getMessage()));
    }
  }
  ...
  @Operation(
      ...
      description = "Retrieves all trips associated with a specific vehicle by its VIN number")
  @ApiResponses(
      value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<ApiResponse<MessageResponse>> startTrip(
      @Parameter(description = "The unique ID of the request assigned with trip", required = true)
          @PathVariable
          Long requestId) {
    try {
      tripService.startTrip(requestId);
      return ResponseEntity.ok(
          ApiResponse.createSuccessResponse(new MessageResponse("Trip started successfully!")));
    } catch (Exception e) {
      return ResponseEntity.badRequest()
  ...
  public ResponseEntity<ApiResponse<MessageResponse>> endTrip(
    ...
    try {
      ...
      return ResponseEntity.ok(
          ApiResponse.createSuccessResponse(new MessageResponse("Trip ended successfully!")));
    } catch (Exception e) {
      return ResponseEntity.badRequest()
